...
iOS sets StreamEx Referer and proxies HLS locally. Browsers cannot. This Worker is the web equivalent of `LocalStreamProxy` + `LiveWebEmbedPlayerView` Referer injection.
...
| `GET /api/embed/player` | Fetch embed HTML with StreamEx Referer, strip ads, inject autoplay |
| `GET /api/embed/hls` | Proxy m3u8/ts with Exposestrat Referer/Origin |
...
Without `NEXT_PUBLIC_FOTTY_API_BASE`, Watch falls back to direct `embed.st` iframes (same failure mode as before: HLS often blocked).
...
- `FOTTY_PLAYBACK_OPEN=1` leaves playback ungated for companion Watch; tighten later if needed.
