...
Generated: 2026-08-30 15:07:16 AST
...
- **Evidence/boundaries**: Add a unit regression proving active PiP remains `.playing(.native)`, retains its background policy and does not mark itself for resume after backgrounding. The full Catalyst suite and unsigned generic iOS Release pass without a simulator; protected PR #8 is merged to `main`. After explicit owner authorization, current `main` commit `0d1085f` was signed, installed only on Jet iPad as a direct Debug 2.0.0 (46), launched and held for 60 seconds; the bounded build output was deleted. This replaces the exact TestFlight binary on that iPad and certifies installation/process health only. It cannot prove background scheduling, a header-proxy stream or system PiP controls; repeat with a native stream by opening another app for 60 seconds and locking/unlocking. No TestFlight upload, version bump, iPhone change or simulator.
